...
public class Transform {
    ...
    public static int[][] rotate(final int[][] originalImage, boolean isLeft, boolean isRight){
        ...
        for (double y = 0; y < originalImage.length; y++) {
            for (double x = 0; x < originalImage[0].length; x++) {
                double[] centeredCoor =
                        center(x, y, originalImage.length, originalImage[0].length);
                x = centeredCoor[0];
                y = centeredCoor[1];
                double originalX = 0;
                double originalY = 0;
                ...
                if (0 <= originalY && originalY < originalImage.length
                        && originalX < originalImage[0].length && 0 <= originalX) {
                    shiftedImage[(int) x][(int) y] =
                            originalImage[(int) originalX][(int) originalY] = 0;
                } else {
    ...
    public static int[][] flip(final int[][] originalImage, boolean isVertical, boolean isHorizontal){
        int[][] shiftedImage = new int[originalImage.length][originalImage[0].length];
        for (double x = 0; x < originalImage.length; x++) {
            for (double y = 0; y < originalImage[0].length; y++) {
                double[] centeredCoor =
                        center(x, y, originalImage.length, originalImage[0].length);
                x = centeredCoor[0];
                y = centeredCoor[1];
                ...
                double[] coor = undoCenter(flippedX, flippedY,
                        originalImage[0].length, originalImage.length);
                flippedX = coor[0];
                flippedY = coor[1];
                if (0 <= flippedY && flippedY <= originalImage.length
                        && flippedX <= originalImage.length && 0 <= originalImage[0].length) {
                    shiftedImage[(int) flippedX][(int) flippedY] = originalImage[(int) x][(int) y];
                } else {
                    shiftedImage[(int) flippedX][(int) flippedY] = FILL_VALUE;
                }
            }
        }
        return shiftedImage;
