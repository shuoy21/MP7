...
public final class QuizMaster {
    ...
    /*public static int computeScore(final int zipCode, final Boolean diversityAnswerCorrect,
        ...
        final int totalScore = 40;
        final int bonusIllinois = 5;

        // calculate scores

        boolean[] questionCorrect = {diversityAnswerCorrect, illiacAnswerCorrect,
                mosaicAnswerCorrect, variableAnswerCorrect};
        for (int i = 0; i < questionCorrect.length; i++) {
            if (questionCorrect[i]) {
                score = score + scoreEachQuestion;
            }
        }
        System.out.println(score);
        // test school

        final int minMichiganZip = 48103, maxMichigan = 48113;
        final int minIllinoisZip = 61801, maxIllinoisZip = 61820;

        if (zipCode >= minMichiganZip && zipCode <= maxMichigan && score != totalScore) {
            score = 0;
        }

        if (zipCode <= maxIllinoisZip && zipCode >= minIllinoisZip
                && score + bonusIllinois <= totalScore) {
            score = score + bonusIllinois;
        }
        return score;
    }*/
    ...
    public static int computeScore(final int zipCode, final Boolean diversityAnswerCorrect,
        ...
        if (48103 <= zipCode && zipCode <= 48113 & score != 40) {
            score = 0;
        }
        if (61801 <= zipCode && zipCode <= 61820 && score + 5 <= 40) {
            score += 5;
        }
        return score;
    }
