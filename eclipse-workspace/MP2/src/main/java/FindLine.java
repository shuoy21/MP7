...
public class FindLine {
   ...
   /* public static void printMatchingLines(final String searchWord, final String corpus) {
        ...
        String word;
        Scanner geneWord;
        Scanner geneLine = new Scanner(corpus); // gene for next line
        while (geneLine.hasNextLine()) {
            printedLine = geneLine.nextLine(); // get the line
            //geneWord = new Scanner(printedLine);
            String curLine = printedLine.replaceAll("[.,!?;()-]", " "); // take out punctruations
            geneWord = new Scanner(curLine);
            while (geneWord.hasNext()) {
                word = geneWord.next();
                if (word.equalsIgnoreCase(searchWord)) {
                    System.out.println(lineCount + ": " + printedLine);
                    break;
                }
            }
            lineCount++;
